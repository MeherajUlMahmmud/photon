...
- **Shortcuts**: every in-app binding is an action in `renderer/src/lib/keymap.ts` (id, scope, default accelerator); components listen through `useShortcut(id, handler)` / `useKeymap().matches(id, e)` from `hooks/use-keymap.tsx`, never raw key checks. User overrides live in main (`main/shortcuts.ts`, `userData/shortcuts.json`) and are broadcast to every window; the two global shortcuts stay in `companion.json`. Settings > Shortcuts edits all of them, refusing same-scope conflicts.
