...
/** Source with line numbers; lines never wrap. */
export function CodeViewer({ content, compact }: ViewerProps & { content: string }) {
  const lines = React.useMemo(() => content.split("\n"), [content]);
  return (
    <pre
      className={cn("grid grid-cols-[auto_1fr] font-mono leading-[1.6]", compact ? "py-2 text-xs" : "py-3 text-small")}
    >
      {lines.map((line, i) => (
        <React.Fragment key={i}>
          ...
          <code className={cn("whitespace-pre", compact ? "pr-3" : "pr-6")}>{line}</code>
        </React.Fragment>
      ))}
    </pre>
  );
